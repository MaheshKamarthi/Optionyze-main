...
export interface RollingOptionsStrangleLiveRuntimeRecord {
    userId: string;
...
interface RollingOptionsStrangleLiveRuntimeRow {
    user_id: string;
    status: "idle" | "running" | "stopped" | "error" | "paused";
...
const gRuntimeFile = path.resolve(process.cwd(), "data", "rolling-options-strangle-live", "runtime.json");

async function loadAllJson(): Promise<RollingOptionsStrangleLiveRuntimeRecord[]> {
    return readJsonFile<RollingOptionsStrangleLiveRuntimeRecord[]>(gRuntimeFile, []);
}

function mapRow(pRow: RollingOptionsStrangleLiveRuntimeRow): RollingOptionsStrangleLiveRuntimeRecord {
    return {
...
export async function listRollingOptionsStrangleLiveRuntime(): Promise<RollingOptionsStrangleLiveRuntimeRecord[]> {
    if (isPostgresConfigured()) {
        const objPool = getPostgresPool();
        const objResult = await objPool.query<RollingOptionsStrangleLiveRuntimeRow>(`
...
export async function loadRollingOptionsStrangleLiveRuntime(pUserId: string): Promise<RollingOptionsStrangleLiveRuntimeRecord | null> {
...
export async function saveRollingOptionsStrangleLiveRuntime(
    pRuntime: RollingOptionsStrangleLiveRuntimeRecord
): Promise<RollingOptionsStrangleLiveRuntimeRecord> {
    const objRuntime: RollingOptionsStrangleLiveRuntimeRecord = {
        ...pRuntime,
