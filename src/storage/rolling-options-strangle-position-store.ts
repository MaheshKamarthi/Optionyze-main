...
export interface RollingOptionsStranglePositionRecord {
    positionId: string;
    userId: string;
    groupId: string;
    cycleId: string;
...
interface RollingOptionsStranglePositionRow {
    position_id: string;
    user_id: string;
    group_id: string;
    cycle_id: string;
...
function mapRowToPosition(pRow: RollingOptionsStranglePositionRow): RollingOptionsStranglePositionRecord {
    return {
        ...
        instrumentType: String(pRow.instrument_type || "OPTION") === "FUTURE" ? "FUTURE" : "OPTION",
        optionSide: (String(pRow.option_side || "") || "") as any,
        action: (String(pRow.action || "") || "") as any,
        strike: pRow.strike === null || pRow.strike === undefined ? null : Number(pRow.strike),
        expiryDate: String(pRow.expiry_date || ""),
        qty: Number(pRow.qty || 0),
        lotSize: Number(pRow.lot_size || 0),
        entryPrice: pRow.entry_price === null || pRow.entry_price === undefined ? null : Number(pRow.entry_price),
        exitPrice: pRow.exit_price === null || pRow.exit_price === undefined ? null : Number(pRow.exit_price),
        markPrice: pRow.mark_price === null || pRow.mark_price === undefined ? null : Number(pRow.mark_price),
        entryDelta: pRow.entry_delta === null || pRow.entry_delta === undefined ? null : Number(pRow.entry_delta),
        exitDelta: pRow.exit_delta === null || pRow.exit_delta === undefined ? null : Number(pRow.exit_delta),
...
export async function listRollingOptionsStrangleOpenPositions(pUserId: string): Promise<RollingOptionsStranglePositionRecord[]> {
    if (isPostgresConfigured()) {
        const objPool = getPostgresPool();
        const objResult = await objPool.query<RollingOptionsStranglePositionRow>(`
            SELECT *
            FROM optionyze_rolling_options_strangle_positions
            WHERE user_id = $1
              AND status = 'OPEN'
...
export async function listRollingOptionsStrangleClosedPositions(
    pUserId: string,
    pFilters?: { fromDate?: string; toDate?: string; }
): Promise<RollingOptionsStranglePositionRecord[]> {
...
export async function saveRollingOptionsStranglePosition(
    pPosition: RollingOptionsStranglePositionRecord
): Promise<RollingOptionsStranglePositionRecord> {
    const vNow = new Date().toISOString();
    const objPosition: RollingOptionsStranglePositionRecord = {
        ...pPosition,
        positionId: String(pPosition.positionId || crypto.randomUUID()),
    ...
    if (isPostgresConfigured()) {
        const objPool = getPostgresPool();
        await objPool.query(`
            INSERT INTO optionyze_rolling_options_strangle_positions (
                position_id,
                user_id,
                group_id,
                cycle_id,
        ...
        `, [
            ...
            objPosition.openedAt ? objPosition.openedAt : vNow,
            objPosition.closedAt ? objPosition.closedAt : null,
...
export async function deleteRollingOptionsStrangleOpenPosition(pUserId: string, pPositionId: string): Promise<boolean> {
    ...
    if (isPostgresConfigured()) {
        ...
        const objResult = await objPool.query(`
            DELETE FROM optionyze_rolling_options_strangle_positions
            WHERE user_id = $1
              AND position_id = $2
              AND status = 'OPEN'
        `, [pUserId, vPositionId]);
    ...
    const objRemainingRows = objRows.filter((objRow) => !(objRow.userId === pUserId && objRow.positionId === vPositionId && objRow.status === "OPEN"));
    await writeJsonFileAtomic(gPositionsFile, objRemainingRows);
    return objRemainingRows.length !== vBeforeCount;
}

export async function clearRollingOptionsStrangleClosedPositions(pUserId: string): Promise<number> {
    if (isPostgresConfigured()) {
        const objPool = getPostgresPool();
        const objResult = await objPool.query(`
            DELETE FROM optionyze_rolling_options_strangle_positions
...
export async function clearRollingOptionsStrangleOpenPositions(pUserId: string): Promise<number> {
    if (isPostgresConfigured()) {
        const objPool = getPostgresPool();
        const objResult = await objPool.query(`
            DELETE FROM optionyze_rolling_options_strangle_positions
    ...
    const objRows = await loadAllPositionsJson();
    const vBeforeCount = objRows.length;
    const objRemainingRows = objRows.filter((objRow) => !(objRow.userId === pUserId && objRow.status === "OPEN"));
    await writeJsonFileAtomic(gPositionsFile, objRemainingRows);
    return vBeforeCount - objRemainingRows.length;
}

export type RollingOptionsPtDePositionRecord = RollingOptionsStranglePositionRecord;

export async function listRollingOptionsPtDeOpenPositions(pUserId: string): Promise<RollingOptionsPtDePositionRecord[]> {
    return listRollingOptionsStrangleOpenPositions(pUserId);
}
...
export async function listRollingOptionsPtDeClosedPositions(
    ...
    return listRollingOptionsStrangleClosedPositions(pUserId, pFilters);
}

export async function saveRollingOptionsPtDePosition(
    pPosition: RollingOptionsPtDePositionRecord
): Promise<RollingOptionsPtDePositionRecord> {
    return saveRollingOptionsStranglePosition(pPosition);
}

export async function deleteRollingOptionsPtDeOpenPosition(pUserId: string, pPositionId: string): Promise<boolean> {
    return deleteRollingOptionsStrangleOpenPosition(pUserId, pPositionId);
}

export async function clearRollingOptionsPtDeClosedPositions(pUserId: string): Promise<number> {
    return clearRollingOptionsStrangleClosedPositions(pUserId);
}
